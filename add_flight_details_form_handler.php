		...
		<?php
			if(isset($_POST['Submit']))
			{
				...
				if(empty($data_missing))
				{
					...
					$query="SELECT count(*) FROM Jet_Details WHERE Aircraft_id=? and active='Yes'";
					...
					if($cnt==1)
					{
						$query="INSERT INTO Flight_Details (flight_no,from_city,to_city,departure_date,arrival_date,departure_time,arrival_time,seats_economy,seats_business,price_economy,price_business,Aircraft_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
						$stmt=mysqli_prepare($dbc,$query);
